package net.luna.common.view.progress;

import net.luna.commom.R;
import net.luna.common.util.Utils;
import net.luna.common.view.CustomView;
...
public class Slider extends CustomView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (value == min) {
			...
			temp.drawCircle(ViewHelper.getX(ball) + ball.getWidth() / 2, ViewHelper.getY(ball) + ball.getHeight() / 2,
			...
			canvas.drawLine(getHeight() / 2, getHeight() / 2, value * division + getHeight() / 2, getHeight() / 2,
					paint);
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		RelativeLayout.LayoutParams params = new LayoutParams(Utils.dpToPx(20, getResources()), Utils.dpToPx(20,
				getResources()));
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			canvas.drawCircle(ViewHelper.getX(ball) + Utils.getRelativeLeft((View) ball.getParent()) + ball.getWidth()
					/ 2, y, size, paint);
			if (animate && size >= finalSize)
				animate = false;
			if (animate == false) {
				ViewHelper.setX(numberIndicator.numberIndicator,
						(ViewHelper.getX(ball) + Utils.getRelativeLeft((View) ball.getParent()) + ball.getWidth() / 2)
								- size);
	...
	class NumberIndicator extends Dialog {
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			indicator.setLayoutParams(new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT,
